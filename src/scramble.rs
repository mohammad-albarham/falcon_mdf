...
pub fn scramble_file<P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    for (address, bytes, changed) in &patches.writes {
        out.seek(SeekFrom::Start(*address))?;
        out.write_all(bytes)?;
        report.blocks_scrambled += 1;
        report.bytes_scrambled += *changed;
    }
    out.flush()?;
...
struct Patches {
    /// Where to write, what to write, and how many of those bytes are text.
    writes: Vec<(u64, Vec<u8>, u64)>,
...
fn plan_patches<S: ByteSource>(source: &S, seed: u64) -> Result<Patches> {
    ...
    for block in &map.blocks {
        ...
        if changed > 0 {
            writes.push((start, replacement, changed));
        }
    }
...
mod tests {
    ...
    fn a_multibyte_name_stays_the_same_number_of_bytes() {
        // "Temperatur°C" is 13 bytes for 12 characters.
        let data = "Temperatur°C\0".as_bytes();
        let (out, changed) = scramble_text(data, &mut Rng::new(2));

        assert_eq!(out.len(), data.len());
        assert_eq!(changed, 12, "the degree sign counts as its two bytes");
        assert!(
            std::str::from_utf8(&out[..12]).is_ok(),
