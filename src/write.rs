...
impl Mf4Writer {
    ...
    pub fn write<W: Write>(&self, out: &mut W) -> Result<()> {
        ...
        let payloads: Vec<Payload> = self
            .groups
            .iter()
            .map(|g| Payload::build(g, self.compress, self.codec))
            .collect::<Result<_>>()?;
...
impl Payload {
    ...
    fn build(group: &WriteGroup, compress: bool, codec: WriteCodec) -> Result<Self> {
        let raw = record_bytes(group);
        if !compress {
            return Ok(Payload::Plain(raw));
        }
        let col_size = record_size(group) as usize;
