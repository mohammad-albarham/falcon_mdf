...
pub fn write_parquet_with<W: Write + Send>(
    ...
    let batch = RecordBatch::try_new(schema.clone(), columns)
        .map_err(|e| Mf4Error::write_error(format!("could not assemble the Parquet table: {e}")))?;
    ...
    let mut writer = ArrowWriter::try_new(out, schema, Some(properties))
    ...
    for s in &series[1..] {
        if s.timestamps() != first.timestamps() {
            return Err(Mf4Error::write_error(format!(
                "Parquet export needs one time axis for the whole table, but '{}' has {} \
