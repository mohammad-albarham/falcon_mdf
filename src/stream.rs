...
impl Mf4File {
    ...
    /// Reads multiple channels from the same channel group a bounded window at a time.
    ///
    /// Yields successive [`Vec<Signal>`] windows of `chunk_size` samples (with the final
    /// window carrying any uneven tail). Every channel's slice in the returned chunk
    /// covers the exact same sample index range.
    ///
    /// # Errors
    ///
    /// Channels from different channel groups are refused by name, because they have
    /// different timebases and cannot be aligned by sample index.
    pub fn signals_chunks<C: std::borrow::Borrow<Channel>>(
        &self,
        channels: &[C],
        chunk_size: usize,
    ) -> Result<SignalsChunks<'_>> {
        if chunk_size == 0 {
            return Err(Mf4Error::parse_error("chunk_size must be greater than 0"));
        }
        if channels.is_empty() {
            return Ok(SignalsChunks::empty(self));
        }

        let first = channels[0].borrow();
        for ch in &channels[1..] {
            let ch = ch.borrow();
            if ch.data_group_index != first.data_group_index
                || ch.channel_group_index != first.channel_group_index
            {
                return Err(Mf4Error::unsupported(
                    "aligned streaming across channel groups",
                    format!(
                        "channel '{}' (group {}:{}) and channel '{}' (group {}:{}) belong to different channel groups and cannot be aligned by index",
                        first.name, first.data_group_index, first.channel_group_index,
                        ch.name, ch.data_group_index, ch.channel_group_index,
                    ),
                ));
            }
        }
        ...
        for ch in channels {
            ...
            if let Some(reason) = ch.unreadable() {
                return Err(Mf4Error::Unsupported {
                    feature: reason.feature().to_string(),
                    detail: format!("reading channel '{}' block by block", ch.name),
                });
            }
        }

        let dg = &self.data_groups()[first.data_group_index];
        let cg = &dg.channel_groups[first.channel_group_index];

        let mut vlsd_group_id = None;
        let mut sd_indices = Vec::with_capacity(channels.len());

        for ch in channels {
            let ch = ch.borrow();
            if ch.channel_type == ChannelType::VariableLength {
                let link = ch.data_link();
                if link == 0 {
                    return Err(Mf4Error::unsupported(
                        "variable-length signal data (VLSD)",
                        format!("channel '{}' has no signal-data link", ch.name),
                    ));
                }
                let group = dg
                    .channel_groups
                ...
                match group {
                    Some(group) => {
                        vlsd_group_id = Some(group.record_id());
                        sd_indices.push(None);
                    }
                    None => {
                        let index = self.build_data_block_index_at(link)?;
                        sd_indices.push(Some(index));
                    }
                }
            } else {
                sd_indices.push(None);
        ...
        let (mode, layout) = if dg.record_index.is_some() {
            ...
            if vlsd_group_id.is_some() {
                return Err(Mf4Error::Unsupported {
                    feature: "block-by-block reading of variable-length signal data".to_string(),
                    detail: format!(
                        "channel group '{}' has a companion payload group in a sorted data group",
                        cg.acquisition_name
                    ),
                });
            }
        ...
        let master = cg.master_channel().cloned();
        let channels_vec = channels.iter().map(|c| c.borrow().clone()).collect();
    ...
    /// Alias for [`signals_chunks`](Self::signals_chunks).
    pub fn aligned_signal_chunks<C: std::borrow::Borrow<Channel>>(
        &self,
        channels: &[C],
        chunk_size: usize,
    ) -> Result<SignalsChunks<'_>> {
        self.signals_chunks(channels, chunk_size)
    }
}

/// An aligned, multi-channel streaming iterator over a channel group's record stream.
///
/// Yields successive windows of [`Vec<Signal>`], where each channel's slice covers
/// the exact same sample index range with bounded memory.
///
/// Created by [`Mf4File::signals_chunks`](crate::Mf4File::signals_chunks).
pub struct SignalsChunks<'a> {
    file: &'a Mf4File,
...
pub type AlignedSignalChunks<'a> = SignalsChunks<'a>;

impl<'a> SignalsChunks<'a> {
    fn empty(file: &'a Mf4File) -> Self {
        Self {
            file,
            channels: Vec::new(),
            layout: RecordLayout {
    ...
    /// Returns the current sample offset in the stream.
    pub fn sample_offset(&self) -> usize {
        self.sample_offset
    }

    /// Returns the master channel of the channel group, if present.
    pub fn master_channel(&self) -> Option<&Channel> {
        self.master.as_ref()
    }

    /// Builds [`SignalSeries`](crate::time_ops::SignalSeries) for a slice of decoded [`Signal`]s
    /// yielded by this iterator.
    ///
    /// Attaches the channel group's master timestamps (or sequential sample indices) for the given
    /// window covering `count` samples starting at `sample_offset`.
    pub fn signals_to_series(
        &self,
        signals: &[Signal],
        sample_offset: usize,
        ...
        signals
            .iter()
            .map(|s| {
                crate::time_ops::SignalSeries::new(
                    s.channel.clone(),
                    timestamps.clone(),
                    s.values()?,
                    s.validity(),
...
impl std::fmt::Debug for SignalsChunks<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SignalsChunks")
            .field(
                "channels",
...
impl Iterator for SignalsChunks<'_> {
    type Item = Result<Vec<Signal>>;

    fn next(&mut self) -> Option<Self::Item> {
        if self.remaining == 0 || self.channels.is_empty() || self.chunk_size == 0 {
            return None;
        }

        let stride = self.layout.record_size;
        if stride == 0 {
            return Some(Err(Mf4Error::parse_error("record stride is zero")));
        }

        let target_samples = self.chunk_size.min(self.remaining);
        let target_bytes = target_samples * stride;

        while self.record_buffer.len() < target_bytes {
            ...
            match &self.mode {
                Mode::Sorted => {
                    let whole = buffer.len() / stride;
                    if whole == 0 {
                        self.carry = buffer;
                        continue;
                    }
                    let kept = whole * stride;
                    self.carry = buffer[kept..].to_vec();
                    self.record_buffer.extend_from_slice(&buffer[..kept]);
                }
                Mode::Unsorted(demux) => {
                    let demuxed = demux.collect(&buffer);
                    let whole = demuxed.records.len() / demux.payload;
                    ...
                    self.record_buffer.extend_from_slice(&demuxed.records);
                }
            }
        }

        let available_samples = self.record_buffer.len() / stride;
        if available_samples == 0 {
            return None;
        }

        let count = available_samples.min(target_samples);
        let take_bytes = count * stride;
        let records = self.record_buffer.drain(..take_bytes).collect::<Vec<u8>>();
        let raw_data = Arc::new(records);
        self.remaining -= count;
        ...
        for (i, channel) in self.channels.iter().enumerate() {
            if let Some(sd_index) = &self.sd_indices[i] {
                ...
                signals.push(signal);
            } else if let Some(payloads) = &self.current_vlsd_payloads {
                if channel.channel_type == ChannelType::VariableLength {
                    let mut signal =
                        Signal::new(channel.clone(), raw_data.clone(), self.layout, count);
                    signal.attach_payloads(payloads.clone());
                    signals.push(signal);
                } else {
                    match self
                        .file
                        .signal_over(channel, raw_data.clone(), self.layout, count)
                    {
