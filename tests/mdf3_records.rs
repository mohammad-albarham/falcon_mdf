...
fn samples_asammdf_wrote_read_back_sample_for_sample() {
    ...
    for version in ["3.30", "3.20", "2.14"] {
        ...
        for (g, dg) in file.data_groups().iter().enumerate() {
            for (c, cg) in dg.channel_groups.iter().enumerate() {
                for (i, channel) in cg.channels.iter().enumerate() {
                    ...
                    let got = file.channel_values(g, c, i).unwrap_or_else(|e| {
                        panic!("{version}: reading {} failed: {e}", channel.name)
                    });
                    assert_eq!(
                        got.len(),
                        10,
        ...
        assert_eq!(
            seen,
            expected.len(),
            "{version}: every channel should be read"
        );
    }
}
...
fn the_time_channel_is_found_and_decodes_to_its_stored_type() {
    ...
    let master = file
        .master_channel(0, 0)
        .expect("the group has a time channel");
    ...
    let index = file.data_groups()[0].channel_groups[0]
        ...
        .unwrap();
    let got = file
        .channel_values(0, 0, index)
        .expect("reading the time channel");
    let want = expected
        .as_array()
        .unwrap()
        .iter()
...
fn non_byte_aligned_fields_decode_in_both_byte_orders() {
    ...
    assert_eq!(
        ints(&file, "LeU12"),
        v.le_u,
        "little-endian unsigned 12-bit fields"
    );
    assert_eq!(
        ints(&file, "LeI12"),
        v.le_i,
        "little-endian signed 12-bit fields"
    );
    assert_eq!(
        ints(&file, "BeU12"),
        v.be_u,
        "big-endian unsigned 12-bit fields"
    );
    assert_eq!(
        ints(&file, "BeI12"),
        v.be_i,
        "big-endian signed 12-bit fields"
    );
...
fn two_group_file(record_id_count: u16) -> (Vec<u8>, Vec<u32>, Vec<i32>) {
    ...
    let ga = Grp {
        record_id: 1,
        record_size: 12,
        channels: vec![Ch::new("time_a", 1, 0, 64, 3), Ch::new("A", 0, 64, 32, 13)],
        records: a
            .iter()
            .enumerate()
            .map(|(i, &v)| {
    ...
    let gb = Grp {
        record_id: 2,
        record_size: 10,
        channels: vec![Ch::new("time_b", 1, 0, 64, 3), Ch::new("B", 0, 64, 16, 14)],
        records: b
            .iter()
            .enumerate()
            .map(|(i, &v)| {
...
fn a_channel_that_does_not_fit_its_record_is_refused_by_name() {
    ...
    for (start, bits, size) in [
        ...
        let path = write_synthetic(dir.path(), &format!("overrun_{start}_{bits}.mdf"), &bytes);
...
fn a_channel_that_exactly_fills_its_record_is_read() {
    ...
    let file = Mdf3File::open(&path).expect("falcon should open it");
    assert_eq!(
        file.values_by_name("X")
            .expect("a channel ending on the last bit fits")
            .len(),
        3
    );
}
...
fn a_data_block_shorter_than_the_declared_cycles_is_refused_by_name() {
    ...
    let err = match file.values_by_name("X") {
        Ok(v) => panic!(
            "a truncated record stream must be refused, got {} samples",
            v.len()
        ),
        Err(e) => e,
    };
...
fn a_record_carrying_an_unclaimed_identifier_is_refused_by_name() {
    ...
    assert_eq!(
        bytes[last], 1,
        "the stream should end with a group 1 record"
    );
    bytes[last] = 9;
    ...
    let err = match file.values_by_name("A") {
        Ok(v) => panic!(
            "an unknown record identifier must be refused, got {} samples",
            v.len()
        ),
        Err(e) => e,
    };
    ...
    assert!(
        ...
        "the refusal should say which one: {err}"
    );
}
...
fn a_trailing_identifier_that_does_not_match_is_refused_by_name() {
    ...
    let err = match file.values_by_name("A") {
        Ok(v) => panic!(
            "a mismatched trailing identifier must be refused, got {} samples",
            v.len()
        ),
        Err(e) => e,
    };
...
fn synthetic_file_with_vax_float_channels_decodes_correctly() {
    ...
    let values_f = file_f
        .values_by_name("vax_f")
        .expect("channel should decode");
    assert_eq!(
        values_f,
        falcon_mdf::SignalValues::F64(vec![1.0, 0.5, -2.5])
    );
    ...
    let grp_d = Grp {
        record_id: 0,
        record_size: 16,
        channels: vec![Ch::new("time", 1, 0, 64, 3), Ch::new("vax_d", 0, 64, 64, 5)],
        records: vec![
            [0u8; 8]
                .into_iter()
                .chain([0x80, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                .collect(),
            [0u8; 8]
                .into_iter()
                .chain([0x20, 0xC1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                .collect(),
        ],
    };
    ...
    let grp_g = Grp {
        record_id: 0,
        record_size: 16,
        channels: vec![Ch::new("time", 1, 0, 64, 3), Ch::new("vax_g", 0, 64, 64, 6)],
        records: vec![
            [0u8; 8]
                .into_iter()
                .chain([0x10, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                .collect(),
            [0u8; 8]
                .into_iter()
                .chain([0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                .collect(),
            [0u8; 8]
                .into_iter()
                .chain([0x24, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
                .collect(),
        ],
    };
