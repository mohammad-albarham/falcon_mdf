...
fn resolve_v4<'a>(file: &'a Mf4File, selector: &str) -> Result<&'a Channel, Mf4Error> {
    if let Some((dg, cg, ch)) = parse_location(selector) {
        return file
            .data_groups()
            .get(dg)
            .and_then(|g| g.channel_groups.get(cg))
    ...
    match matches.as_slice() {
        ...
        _ => Err(Mf4Error::parse_error(format!(
            "ambiguous channel '{selector}'; use its location id from channels()"
        ))),
    }
}
...
fn resolve_v3(file: &Mdf3File, selector: &str) -> Result<(usize, usize, usize), Mf4Error> {
    if let Some((dg, cg, ch)) = parse_location(selector) {
        ...
        return Err(Mf4Error::ChannelNotFound {
            name: selector.into(),
        });
    }
    let mut found = None;
                ...
                if channel.name == selector {
                    if found.is_some() {
                        return Err(Mf4Error::parse_error(format!(
                            "ambiguous channel '{selector}'; use its location id"
                        )));
                    }
                    found = Some((dg, cg, ch));
                }
    ...
    found.ok_or_else(|| Mf4Error::ChannelNotFound {
        name: selector.into(),
    })
}
    ...
    fn channel_kind(&self, name: &str) -> Option<&'static str> {
        match self {
            ...
            Inner::V3(f) => {
                ...
                Some(
                    match f.data_groups()[dg].channel_groups[cg].channels[ch].data_type {
                        7 => "text",
                        8 => "bytes",
                        _ => "f64",
                    },
            ...
            + match &self.payload {
                Payload::Text(labels) => {
                    labels.capacity() * std::mem::size_of::<Option<String>>()
                        + labels.iter().flatten().map(|s| s.capacity()).sum::<usize>()
                }
                ...
                Payload::VarBytes { data, starts } => {
                    data.capacity() + starts.capacity() * std::mem::size_of::<usize>()
                }
                _ => 0,
            }
    }
...
fn write_label(out: &mut String, label: Option<&str>) {
    if let Some(label) = label {
        out.push('"');
        escape_json_str_into(label, out);
        out.push('"');
    } else {
        out.push_str("null");
    }
...
fn write_sample(
    ...
    let valid = |j| {
        validity.is_none_or(|v| {
            v.get(if v.len() == values.len() { i } else { j })
                .copied()
                .unwrap_or(false)
        })
    };
    match values {
        ...
        SignalValues::Array {
            ...
            let a = i * elements_per_sample;
            let b = a + elements_per_sample;
            write_array_sample(out, flat, a, b, element, valid);
        }
        SignalValues::ArrayVarLen {
            values: flat,
            starts,
        } => write_array_sample(out, flat, starts[i], starts[i + 1], element, valid),
        SignalValues::Bytes { data, width } => {
            if valid(i) {
                hex_field(&data[i * width..(i + 1) * width], out);
            } else {
                out.push_str("null");
            }
        }
        SignalValues::VarBytes { data, starts } => {
            if valid(i) {
                hex_field(&data[starts[i]..starts[i + 1]], out);
            } else {
                out.push_str("null");
            }
        }
        _ => write_f64(
            out,
            if valid(i) {
                values.to_f64().get(i).copied().unwrap_or(f64::NAN)
            } else {
                f64::NAN
            },
        ),
...
fn write_array_sample(
    ...
    if let Some(element) = element {
        ...
        out.push('[');
        for (j, &value) in flat.iter().enumerate().take(b).skip(a) {
            if j > a {
                out.push(',');
            }
            write_f64(out, if valid(j) { value } else { f64::NAN });
        }
        out.push(']');
    }
}
...
fn append_extent(out: &mut String, extent: Option<(f64, f64)>) {
    out.push_str(",\"tMin\":");
    write_f64(out, extent.map_or(f64::NAN, |e| e.0));
    out.push_str(",\"tMax\":");
    write_f64(out, extent.map_or(f64::NAN, |e| e.1));
}
...
impl WasmMf4File {
    ...
    fn decoded(&mut self, name: &str) -> Result<&CachedSeries, JsValue> {
        let entry = if let Some(pos) = self.series_cache.iter().position(|(n, _)| n == name) {
            ...
            self.fetch_series(name)?
        };
        let bytes = entry.byte_size();
        while !self.series_cache.is_empty()
            && (self.series_cache.len() >= SERIES_CACHE_CAP
                || self
                    .series_cache
                    .iter()
                    .map(|(_, s)| s.byte_size())
    ...
    fn file_channel(&self, name: &str) -> Result<Option<(&Mf4File, &Channel)>, JsValue> {
        if self.bus.iter().any(|b| b.name == name) || self.computed.iter().any(|(n, _)| n == name) {
            return Ok(None);
        }
        match &self.inner {
    ...
    pub fn window_series(
        ...
        if let Some((file, channel)) = self.file_channel(name)? {
            if kind_of_channel(channel) == "text" {
                let window = file
                    .view_text_window(channel, t0, t1, budget)
                    .map_err(js_err)?;
                let mut out = signal_json(name, &channel.unit, &window.timestamps, &[])?;
                out.pop();
                out.push_str(",\"kind\":\"text\",\"labels\":[");
                ...
                append_extent(&mut out, window.extent);
                out.push('}');
                return Ok(out);
            }
            let window = file
                .view_window(channel, element, t0, t1, budget)
                .map_err(js_err)?;
            let mut out = signal_json(name, &channel.unit, &window.timestamps, &window.values)?;
            out.pop();
            let _ = write!(out, ",\"kind\":\"{}\"", kind_of_channel(channel));
            if let Some(shape) = channel.array_shape() {
                let _ = write!(out, ",\"elements\":{}", shape.iter().product::<u64>());
            }
            append_extent(&mut out, window.extent);
            out.push('}');
            return Ok(out);
    ...
    pub fn sample_page(
        ...
        if count > 4096 {
            return Err(js_err("table page exceeds 4096 rows"));
        }
        let Some((file, channel)) = self.file_channel(name)? else {
            ...
            if let Payload::Text(labels) = &series.payload {
                ...
                for (i, label) in labels[start..end].iter().enumerate() {
                    ...
                    write_label(&mut out, label.as_deref());
                }
                out.push(']');
            }
            out.push('}');
        ...
        let total = file.data_groups()[channel.data_group_index].channel_groups
            [channel.channel_group_index]
            ...
            for i in start.saturating_sub(offset)..end.saturating_sub(offset).min(ts.len()) {
                ...
                if rows.len() > 16 * 1024 * 1024 {
                    return Err(Mf4Error::parse_error(
                        "table page exceeds 16 MiB; request fewer rows",
                    ));
                }
            }
    ...
    pub fn sample_at(&mut self, name: &str, time: f64, element: usize) -> Result<String, JsValue> {
        ...
        if let Some((file, channel)) = self.file_channel(name)? {
            ...
            let mut sample = String::from("null");
            file.visit_samples(channel, true, |_, times, values, validity| {
                for (i, &t) in times.iter().enumerate() {
                    ...
                    if d < distance {
                        distance = d;
                        sample.clear();
                        write_sample(&mut sample, values, validity, i, Some(element))?;
                    }
                }
                Ok(true)
            })
        ...
        match &series.payload {
            Payload::Text(labels) => write_label(&mut out, labels[i].as_deref()),
            _ => write_f64(&mut out, series.values.get(i).copied().unwrap_or(f64::NAN)),
        }
        Ok(out)
    }
    ...
    pub fn channels(&self) -> Result<String, JsValue> {
            ...
            Inner::V4(f) => {
                for (dg, group) in f.data_groups().iter().enumerate() {
                    for (cg, group) in group.channel_groups.iter().enumerate() {
                        for (ch, channel) in group.channels.iter().enumerate() {
                            entries.push((
                                location_id(dg, cg, ch),
                                channel.name.clone(),
                                channel.unit.clone(),
                                if group.acquisition_name.trim().is_empty() {
                                    format!("group {dg}.{cg}")
                                } else {
                                    group.acquisition_name.clone()
                                },
                                channel.comment.clone(),
                                kind_of_channel(channel),
                            ));
                        }
                    }
                }
        ...
        for (id, name, unit, group, description, kind) in entries {
            ...
            .enumerate()
            {
                ...
                let _ = write!(out, "\"{key}\":\"");
                escape_json_str_into(value, &mut out);
                out.push('"');
            }
            out.push('}');
        }
